...
function deleteConstraint(button) {
    const constraintDiv = button.parentElement;
    const constraints = document.getElementById('constraints');
    
    // Only delete if there's more than one constraint
    if (constraints.children.length > 1) {
        constraints.removeChild(constraintDiv);
    } else {
...
function addConstraint() {
    ...
    constraintHtml += `
        ...
        <button type="button" class="delete-constraint" onclick="deleteConstraint(this)">Delete</button>
    `;
